...
=head1 SYNOPSIS
  ...
  my $child = $node->appendTextChild( $childname , $PCDATA );
...
Somewhat similar with C<<<<<< appendTextNode >>>>>>: It lets you set an Element, that contains only a C<<<<<< text node >>>>>> directly by specifying the name and the text content. Returns the newly created
child element node.
