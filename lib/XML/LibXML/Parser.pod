...
Unless specified otherwise, the options C<<<<<< load_ext_dtd >>>>>> and C<<<<<< expand_entities >>>>>> are set to 0. See L<<<<<< Parser Options >>>>>> for a list of libxml2 parser's options.
...
substitute entities; possible values are 0 and 1; default is 0
...
Forbid network access; possible values are 0 and 1. Defaults to 0 unless
XML::LibXML was built with C<<<<<< -DLIBXML_DEFAULT_NONET >>>>>> (the C<<<<<< perl Makefile.PL >>>>>> prompt "Enable no_network by default?" was answered yes, or C<<<<<< XML_LIBXML_DEFAULT_NONET=1 >>>>>> was set in the environment), in which case it defaults to 1.
