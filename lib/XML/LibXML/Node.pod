...
=head1 SYNOPSIS
  ...
  $bytes = $node->toBytesC14N();
  $bytes = $node->toBytesC14N($with_comments, $xpath_expression , $xpath_context);
  $bytes = $node->toBytesC14N_v1_1();
  $bytes = $node->toBytesC14N_v1_1($with_comments, $xpath_expression , $xpath_context);
  $bytes = $node->toBytesEC14N();
  $bytes = $node->toBytesEC14N($with_comments, $xpath_expression, $inclusive_prefix_list);
  $bytes = $node->toBytesEC14N($with_comments, $xpath_expression, $xpath_context, $inclusive_prefix_list);
...
Behaves like toStringC14N() but returns the result as a byte string (octets)
with the UTF-8 flag off. This is useful when the canonical form needs to be fed
to a binary consumer such as a digest function (e.g. Digest::SHA) or written to
a binary stream, where Perl's UTF-8 flag could cause unwanted encoding
conversions.


=item toBytesC14N_v1_1

  $bytes = $node->toBytesC14N_v1_1();
  $bytes = $node->toBytesC14N_v1_1($with_comments, $xpath_expression , $xpath_context);

Byte-string variant of toStringC14N_v1_1(). Returns octets with the UTF-8 flag
off.
...
Byte-string variant of toStringEC14N(). Returns octets with the UTF-8 flag off.
