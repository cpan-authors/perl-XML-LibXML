...
This example demonstrates C<<<<<< registerVarLookupFunc() >>>>>> method. We use XPath variables to recycle results of previous evaluations:
...
Registers namespace C<<<<<< $prefix >>>>>> to C<<<<<< $namespace_uri >>>>>>. Note that namespace registrations are per-context: they are not shared
between C<<<<<< XML::LibXML::XPathContext >>>>>> objects. If you create a new context for a sub-tree, you must register all
required namespaces on the new context as well.
