...
I<<<<<< Important note about formatting and whitespace: >>>>>> The formatter only inserts indentation between child elements that are not
already separated by text content. When XML is parsed normally, whitespace
between tags (newlines, spaces used for indentation in the source) is preserved
as text nodes. These existing text nodes prevent the formatter from adding its
own indentation because it will not alter existing text.

To get proper re-indentation, strip whitespace-only text nodes at parse time
using the C<<<<<< no_blanks >>>>>> option:
  ...
  print $doc->toString(1);  # now produces indented output

Without C<<<<<< no_blanks =E<gt> 1 >>>>>>, calling C<<<<<< toString(1) >>>>>> on a document that was parsed with default settings will typically return the
XML unchanged, since the original whitespace text nodes are still present.
