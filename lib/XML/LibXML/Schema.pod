...
of schema. Optional parameter no_network set to 1 causes the parser to forbid
network access (this is already the default when XML::LibXML was built with C<<<<<< LIBXML_DEFAULT_NONET >>>>>>). Optional parameter recover set to 1 causes the parser to not call die() on
errors.
